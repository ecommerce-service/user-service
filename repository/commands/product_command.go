...
func (c ProductCommand) Add() (res string, err error) {
	statement := `INSERT INTO products(category_id,name,sku,price,discount,main_image_key,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`

	err = c.db.GetTx().QueryRow(statement, c.model.CategoryId(), c.model.Name(), c.model.Sku(), c.model.Price(), c.model.Discount().Float64, c.model.MainImageKey().String,
...
func (c ProductCommand) Edit() (err error) {
	statement := `UPDATE products SET category_id=$1,name=$2,sku=$3,price=$4,discount=$5,main_image_key=$6,updated_at=$7 WHERE id=$8 RETURNING id`

	_, err = c.db.GetTx().Exec(statement, c.model.CategoryId(), c.model.Name(), c.model.Sku(), c.model.Price(), c.model.Discount().Float64, c.model.MainImageKey().String,
